...
| `models` | per-slot model block: `{ agent?, paint?, judge? }`, each a model-name string (resolved through the credential ladder — passed verbatim to the resolved provider, pinnable via `VENDO_MODEL` / `VENDO_MODEL_PAINT` / `VENDO_MODEL_JUDGE`) or an explicit AI SDK `LanguageModel` object (wins as-is). `agent` supersedes `model`; `paint` supersedes `paint.model`; `judge` only feeds a judge you wired from `vendoModel("vendo-judge")` — no judge runs by default |
