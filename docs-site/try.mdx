...
[dev-mode credential ladder](/connect/dev-mode) the runtime uses. With zero
keys the surfaces still serve against scripted data, so there is always
something to click.

### Refine in the surface

With a model resolved, the surface takes corrections in conversation: "call
these workspaces, not projects", "the accent is wrong". Proposals arrive as
