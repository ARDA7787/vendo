...
| 3 | Revoke mid-session: reads age, the next write fails on live rows | Yousef shared *Desk ledger* with `user:maple-mia` at *Can edit*; Mia **opened** it; Yousef then hit **Remove** on Mia's row in the Share dialog | Grants → `[]`. Mia's **already-open view keeps rendering** (`03c`). Her next writes fail against live rows: `POST …/edit` **404**, `POST …/history {"op":"undo"}` **404**, and the app is gone from her fresh list (`03d`) — masked, because she is no longer even a viewer. | `03a`–`03e` | **PASS** |
| 4 | An org-shared app's per-user data stays separate | Yousef moved *Desk ledger* into the org (`team:maple/support`, *Can view*); both people opened the **same app id** | One document, one id, `personal:false` — and each person's own row on screen: Yousef sees *Yousef Helal / yousef@maple.com*, Mia sees *Mia Nakamura / mia@maple.com*. The app's query resolves per caller through the ordinary tool pipeline. | `04a`, `04b`, `04c` | **PASS** (see F5 for the half this does not cover) |
...
promise is unreachable in the UI.** `list` in
...
sponsor's Automations tab too.** Same `list`: only `status === "active"` sponsorships
...
**F4 — the fork offer is reachable from exactly one control.** `during(action, appId)`
...
**F6 (cosmetic, host-side) — the account chip shows the wrong initials.** Signed in as
