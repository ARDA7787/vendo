#!/usr/bin/env node
/**
 * THE LIVE PROOF for door-internal: a REAL e2b box, a REAL Claude Agent SDK
 * session inside it, reaching a REAL host over a REAL public URL, through the
 * REAL MCP door, with a credential the host minted for the turn in flight —
 * and the host never passed `mcp`.
 *
 * This is door-ctx's live proof with ONE thing changed: the composition asks
 * for nothing but `harness: claudeCode(...)`. No `mcp`, no `oauth`. What the
 * box dials is the INTERNAL half composition mounted for it, and check 7 walks
 * up to the same public origin as an outside agent would and finds nothing.
 *
 ...
 *
 *   node docs/verification/door-internal/live-internal-door-proof.mjs
 *
 ...
 *   7. the outside is LOCKED OUT on the same public origin: no discovery, no
 *      authorization server, no registration, and a 401 that names no way in
 */
...
const SUBJECT = "user_live_internal_door";
const results = [];
...
// The WHOLE composition. No `mcp`, no `oauth` — naming a harness that thinks
// on a machine is the entire ask, and composition answers with the internal
// half of the door. The operator base is how this deployment names the origin
// its box can dial; it opens nothing.
process.env.VENDO_BASE_URL = publicUrl;
...
try {
  ...
  console.log("\n[proof] 1/7 · a read, through the door, from inside a real box");
  ...
  console.log("\n[proof] 3/7 · a parked write: the card must reach the queue MID-turn");
  ...
  console.log("\n[proof] 4/7 · commit: a file the box wrote is readable through the host");
  await runTurn("thr_live_write", "Create a file at user/notes/proof.md containing exactly the line: DOOR-INTERNAL-4417. Then say DONE.");
  ...
  record(
    ...
    landed.includes("DOOR-INTERNAL-4417"),
    JSON.stringify(landed.slice(0, 120)),
  );

  // ── 5 · unattended, through the same door ───────────────────────────────────
  console.log("\n[proof] 5/7 · an unattended turn is judged ABSENT");
  ...
  console.log("\n[proof] 6/7 · a credential outside a turn is a 401");
  ...
  // ── 7 · the outside, on the very same public origin ─────────────────────────
  console.log("\n[proof] 7/7 · an outside agent walks up to the same URL and finds nothing");
  const absent = [];
  for (const path of [
    ...
    "/api/vendo/mcp/authorize?response_type=code&client_id=x",
    "/api/vendo/mcp/connect",
  ]) {
    const probe = await fetch(`${publicUrl}${path}`);
    absent.push(`${path} → ${probe.status}`);
  }
  const registered = await fetch(`${publicUrl}/api/vendo/mcp/register`, {
    method: "POST",
    headers: { "content-type": "application/json" },
    body: JSON.stringify({ client_name: "an outside agent", redirect_uris: ["https://client.example/cb"] }),
  });
  absent.push(`POST /register → ${registered.status}`);
  record(
    "7 · every outside path on the public origin is 404 — there is no door to find",
    absent.every((entry) => entry.endsWith("404")),
    JSON.stringify(absent),
  );
  record(
    "7 · the mount's 401 names no way in: no www-authenticate challenge at all",
    stale.headers.get("www-authenticate") === null,
    `www-authenticate: ${JSON.stringify(stale.headers.get("www-authenticate"))}`,
  );
} catch (error) {
  failed = true;
  ...
  await writeFile(
    new URL("live-internal-door-proof.json", import.meta.url),
