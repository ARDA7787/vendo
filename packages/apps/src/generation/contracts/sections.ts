...
export const hostToolSections = (deps: GenerationDependencies): GenerationPromptSection[] => [
  ...
  ...(deps.toolShapes === undefined || Object.keys(deps.toolShapes).length === 0 ? [] : [{
    ...
    id: "catalog" as const,
    content: `RESHAPE PIPES — a binding may end with a bounded \`| op(...)\` pipe (this is the ONLY computation allowed in a binding; the closed set is pick, rename, asPoints, format, sum, avg, min, max, count — nothing else, ever: no groupBy, no filter, no map, and never a bare ".length" property, which does not exist on a binding). PREFER native field-name props over pipes: components read RAW tool rows directly — Select takes the raw object array plus labelField/valueField, DataTable/CardList columns resolve dot-path keys ({key:"client.name"}), Kit charts read raw rows via data + xKey + series. Never pre-project rows a component can read raw.
- Need a COUNT of rows for a Stat or number (e.g. "12 issues")? Use the count aggregate, never ".length": value={issues_list({}) | count()}. Need a total/average/min/max of one numeric field across rows? sum/avg/min/max take that field name: value={invoices_list({}) | sum(amountCents)} format="money".
